...
public class AiImpl implements AI {
    ...
    public Step getBestStep(BoardService board, String User, Integer deep) {
        ...
        List<Step> allStep=getAllStep2(board,User);
        Integer score=0xcfcfcfcf;
        if(User.equals("black"))User="red";
        else User="black";
        for (Step step : allStep) {
            fakeMove(board,step);
            Integer tScore=getMaxMinScore(board,User,deep-1,0x3f3f3f3f,0xcfcfcfcf);
            //System.out.println(step+" "+tScore);
            if(tScore>score){
                score=tScore;
                bestStep=step;
            }
    ...
    public List<Step> getAllStep(BoardService board, String user) {
        ...
        for(int i=1;i<=16;i++){
            if(!board.getStone(i+offset).is_isDead()){
                for(int r = 0; r< Board.get_n(); r++){
                    for(int c=0;c<Board.get_m();c++){
                        if(board.isExist(r,c)&&board.isSameColor(board.getStone(i+offset).get_row(),board.getStone(i+offset).get_col(),r,c))continue;
                        if(board.canMove(board.getStone(i+offset),r,c)){
                            score=0;
                            if(board.isExist(r,c)){
                                score=board.getStoneScore(board.getGameMapId(r,c));
                            }
                            stepsList.add(new Step(i+offset,board.getStone(i+offset).get_row(),board.getStone(i+offset).get_col(),board.getGameMapId(r,c),r,c,user,score));
                        }
                    }
                }
            }
    ...
    public Integer getMaxMinScore(BoardService board, String user,Integer deep,Integer alpha,Integer beta) {
        ...
        Integer score;
        List<Step> stepList=getAllStep2(board,user);
    ...
    @Override
    public List<Step> getAllStep2(BoardService board, String user) {
        Integer []max={2,-2,1,-1};
        Integer score=0;
               ...
               if(stone.get_type()==0){//车的所有走法
                   ...
                   for(int j=0;j<2;j++){
                       for(int k=0;k<2;k++){
                           ...
                           eCol=col+max[k];
                           if(board.isExist(eRow,eCol)&&board.isSameColor(row,col,eRow,eCol))continue;
                           if(board.canMoveXiang(user,row,col,row+max[j],col+max[k])){
                               if(board.isExist(eRow,eCol)){
                                   score=board.getStoneScore(board.getGameMapId(eRow,eCol));
                               }
                               stepsList.add(new Step(i,row,col,board.getGameMapId(eRow,eCol),eRow,eCol,user,score));
