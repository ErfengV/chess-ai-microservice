...
public class BoardImpl implements BoardService {
    ...
    public boolean isExist(Integer row,Integer col){
        if(row<0||row>=Board.get_n()||col<0||col>=Board.get_m())return false;
        int stoneId=getGameMapId(row,col);
        if(stoneId!=0&&getStone(stoneId).is_isDead()==false)return true;
        return false;
    }
    ...
    public Stone getStone(Integer row, Integer col) {
        Stone stone=null;
        if(isExist(row,col)==true)stone=getStone(getGameMapId(row,col));
        return stone;
    }
    ...
    public boolean isSameColor(Integer row1, Integer col1, Integer row2, Integer col2) {
        ...
        if(row1<0||row1>=Board.get_n()||col1<0||col1>=Board.get_m())return false;
        if(row2<0||row2>=Board.get_n()||col2<0||col2>=Board.get_m())return false;
        try {
    ...
    public Integer getNumsTwoPoint(Integer row1, Integer col1, Integer row2, Integer col2) {
        ...
        if(row1==row2){
            for(int i=Math.min(col1,col2)+1;i<Math.max(col1,col2);i++){
                if(getGameMapId(row1,i)!=0)num++;
            }
        }
        if(col1==col2){
            for(int i=Math.min(row1,row2)+1;i<Math.max(row1,row2);i++){
                if(getGameMapId(i,col1)!=0)num++;
            }
        }
        return num;
    }
    ...
    public boolean canMoveMa(String user, Integer startRow, Integer startCol, Integer endRow, Integer endCol) {
        if(endRow<0||endRow>=Board.get_n()||endCol<0||endCol>=Board.get_m())return false;
        boolean result=false;
        Integer disRow=startRow-endRow;
        Integer disCol=startCol-endCol;
        if(Math.abs(disRow)==2&&Math.abs(disCol)==1){
            if(disRow<0){
                if(isExist(startRow+1,startCol)==false){
                    result=true;
                }
            }
            else{
        ...
        if(Math.abs(disRow)==1&&Math.abs(disCol)==2){
            if(disCol<0){
                if(isExist(startRow,startCol+1)==false){
                    result=true;
                }
            }
            else{
    ...
    public boolean canMoveXiang(String user, Integer startRow, Integer startCol, Integer endRow, Integer endCol) {
        ...
        if(f*endRow<=f*offset){
            if(Math.abs(disRow)==2&&Math.abs(disCol)==2){
                if(disRow>0){
                    if(disCol>0&&isExist(startRow-1,startCol-1)==false){
                        result=true;
                    }
                    if(disCol<0&&isExist(startRow-1,startCol+1)==false){
                        result=true;
    ...
    public boolean canMoveJiang(String user, Integer startRow, Integer startCol, Integer endRow, Integer endCol) {
        ...
        try{
            Integer diJiangRow=board.get_s()[f*offset+getGameMapId(startRow,startCol)].get_row();
            Integer diJiangCol=board.get_s()[f*offset+getGameMapId(startRow,startCol)].get_col();
            if(startCol==diJiangCol&&endRow==diJiangRow&&endCol==diJiangCol&&getNumsTwoPoint(startRow,startCol,endRow,endCol)==0){
                result=true;
            }
        }
        catch (Exception e){
    ...
    public boolean canMoveBin(String user, Integer startRow, Integer startCol, Integer endRow, Integer endCol) {
        ...
        if(f*startRow<=f*offset){
            if(disRow==-f&&disCol==0)result=true;
        }
        else{
            if(disRow==-f&&disCol==0)result=true;
            if(disRow==0&&Math.abs(disCol)==1)result=true;
        }
        return result;
    }
    ...
    public boolean isGameEnd() {
        if(getStone(5).is_isDead()==true||getStone(21).is_isDead()==true)return true;
        else return false;
    }

    @Override
    public Integer getStoneScore(Integer id) {
        return Stone.stoneScore[getStone(id).get_type()];
    }
